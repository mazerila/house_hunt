...
6. **The agency's own ebook/plaquette is more honest than its portal ad.** Its grid gives **état général / extérieur / intérieur**, **mitoyenneté**, **honoraires** (⇒ net vendeur), and the direct negotiator. H05's portal ad said "Entretenu"; its own ebook said **« Moyen / Moyen / À rafraîchir »**. Quote the ebook, it's the seller's admission.
7. **Audit understatement still applies** (see the DPE section above): the DPE's own work packs are regulatory figures — recost against IdF market rates and **add the non-energy works the pack ignores** (électricité, salles d'eau, rafraîchissement, extérieur under ABF).
8. **Inventory what is MISSING.** H05's DDT had no **ERP (état des risques)** — mandatory at compromis, and the only route to RGA argile / carrières / CatNat. Also chase taxe foncière, ASL charges, and any combles-conversion authorisation. List these in `## Open items` and tell the user to demand them.
9. **Then rewrite the dossier honestly**: add a **`## Documents au dossier`** section (each file, what it is, its date + operator), revise the works table and the verdict, **restate any earlier conclusion the documents overturn** (don't quietly edit it away — H05's verdict explicitly says the "0–40 k€ cosmétique" estimate was wrong and why), reseed `price_min` / `price_offer` via the dashboard API, and update `private/list.md`.
